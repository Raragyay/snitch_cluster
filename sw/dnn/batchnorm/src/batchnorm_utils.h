...
batchnorm_backward_fp64_no_loop(const double* grad_ofmap_scratch,
    ...
    uint32_t work_div_2_sub_1 = num_points_work_for_core / 2 - 1;  // can underflow, but then frep won't happen
        ...
        do {  // while (i < num_channels_to_process)
            ...
            asm volatile(
                ...
                : [buf_flag] "+r"(buf_flag), [invstd_scratch] "+r"(invstd_scratch),
                  ...
                  [prev_work] "+r"(prev_work), [frep] "+r"(frep), [work_div_2_sub_1] "=r"(work_div_2_sub_1),
                ...
                : [i] "r"(i), [num_channels_to_process] "r"(num_channels_to_process),
                  ...
                  [work_div_2_sub_1_offset] "i"(offsetof(dm_comm_t, work_div_2_sub_1)),
                  [REG_BOUNDS_PLUS_0] "i"(REG_BOUNDS), [REG_WPTR_2D] "i"(REG_WPTR + 2), [REG_RPTR_2D] "i"(REG_RPTR + 2),
                  [DM_ALL] "i"(SNRT_SSR_DM_ALL), [DM_0] "i"(SNRT_SSR_DM0), [DM_1] "i"(SNRT_SSR_DM1),
                  [DM_2] "i"(SNRT_SSR_DM2), [REG_STRIDES_PLUS_1] "i"(REG_STRIDES + 1),
                  [inner_loop_stride] "r"(inner_loop_stride), [outer_loop_stride] "r"(outer_loop_stride),
...
static inline void batchnorm_backward_main_loop(uint32_t C, uint32_t work_left,  // only present for dma
    ...
    if (snrt_is_dm_core()) {
        ...
        // align up to multiple of 2 because that avoids stalling in fpu the best
        uint32_t ideal_work_in_tile = min(ALIGN_UP(ceildiv(work_left, min_loops), 2), tile_size_in_points);
        ...
        while (work_left > 0) {
            ...
            DUMP(55);
            snrt_dma_wait_all();
            // wait for previous tile to be finished computing, signify current tile inputs done loading
            snrt_cluster_hw_barrier();
            DUMP(56);
        ...
        if (num_channels_work_for_core == 0) {
            ...
            batchnorm_backward_tile_fp64_looped(
                ...
                initial_work_mod_3, initial_work_div_3_sub_1, tile_size_in_points, num_channels_work_for_core,
