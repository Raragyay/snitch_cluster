#ifdef MAXPOOL_FN_SINGLE_CORE // serves as an include guard
...
#define MAXPOOL_FN_SINGLE_CORE_1D CONCAT(MAXPOOL_FN_SINGLE_CORE, _1D)
#define MAXPOOL_FN_SINGLE_CORE_2D CONCAT(MAXPOOL_FN_SINGLE_CORE, _2D)
#define MAXPOOL_FN_SINGLE_CORE_3D CONCAT(MAXPOOL_FN_SINGLE_CORE, _3D)
#define MAXPOOL_FN_SINGLE_CORE_UNTILED CONCAT(MAXPOOL_FN_SINGLE_CORE, UNTILED)

#ifndef ___MAXPOOL_SINGLE_CORE_C
#define ___MAXPOOL_SINGLE_CORE_C
#define ENABLE_SPECIALIZED_SINGLE_CORE 1

#define DMA_ATTRIBS_SINGLE_CORE 1
#define DMA_INDICES_SINGLE_CORE 1
#define USE_SSR_FREP_1D_SINGLE_CORE 1
#define USE_SSR_FREP_2D_SINGLE_CORE 1
#define USE_SSR_FREP_3D_SINGLE_CORE 1
#define USE_SSR_FREP_ALL_SINGLE_CORE 0
#define USE_DOUBLE_BUFFERING_SINGLE_CORE 1

#define BASE_USABLE_CACHE_SINGLE_CORE 100000

// number of usable bytes in l1
#define USABLE_CACHE_SINGLE_CORE BASE_USABLE_CACHE_SINGLE_CORE

// for 8 byte alignment
#define ATTRIBS_SIZE_SINGLE_CORE ((sizeof(maxpool_attributes) + 4) - ((sizeof(maxpool_attributes) + 4) % 8))

#ifdef DMA_ATTRIBS_SINGLE_CORE
  #undef USABLE_CACHE_SINGLE_CORE
  #define USABLE_CACHE_SINGLE_CORE (BASE_USABLE_CACHE_SINGLE_CORE - ATTRIBS_SIZE_SINGLE_CORE)
#endif

// number of usable bytes per half
#define HALF_CACHE_SINGLE_CORE ((USABLE_CACHE_SINGLE_CORE / 2) - 16)
...
static inline void MAXPOOL_FN_SINGLE_CORE_1D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs);
static inline void MAXPOOL_FN_SINGLE_CORE_2D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs);
static inline void MAXPOOL_FN_SINGLE_CORE_3D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs);

static inline void MAXPOOL_FN_SINGLE_CORE_UNTILED(maxpool_attributes*, double*, double*, int*, int, int, char*);

void MAXPOOL_FN_SINGLE_CORE_UNTILED(maxpool_attributes* attribs,
                        double* in,
                        double* out,
                        int* idx,
                        int total_ins,
  ...
  if (snrt_is_compute_core()) {
    ...
    #if MAXPOOL_DIM == 1
    MAXPOOL_FN_SINGLE_CORE_1D(attribs, (double*) inputs_start, (double*) outputs_start, idx_out, total_outs);
    #elif MAXPOOL_DIM == 2
    MAXPOOL_FN_SINGLE_CORE_2D(attribs, (double*) inputs_start, (double*) outputs_start, idx_out, total_outs);
    #elif MAXPOOL_DIM == 3
    MAXPOOL_FN_SINGLE_CORE_3D(attribs, (double*) inputs_start, (double*) outputs_start, idx_out, total_outs);
    #endif
...
void MAXPOOL_FN_SINGLE_CORE(maxpool_attributes* attribs_raw, double* in, double* out, int* idx) {
  ...
  #if !USE_DOUBLE_BUFFERING_SINGLE_CORE
  MAXPOOL_FN_SINGLE_CORE_UNTILED(attribs, in, out, idx, total_ins, total_outs, ptr);
  #else

  #if (defined(MAXPOOL_ROW_MAJOR) || defined(MAXPOOL_COL_MAJOR)) && DMA_INDICES_SINGLE_CORE
...
void MAXPOOL_FN_SINGLE_CORE_1D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs) {
  ...
  if ((attr->kernel_shape[0] - 1) * attr->dilations[0] + 1 <= attr->input_shape[2]) {
    ...
    if (attr->pads[0] != 0 || attr->pads[1] != 0) {
      ...
      if (attr->pads[0] != 0) {
        ...
        while (padding_left > 0) {
          ...
          snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_2D, in + in_offset);
          snrt_ssr_write(SNRT_SSR_DM1, SNRT_SSR_1D, out + out_offset);
      ...
      if (attr->pads[1] != 0) {
          ...
          for (int i = 0; i < padded_kernels; ++i) {
            ...
            snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_2D, in_copy + offset);
            snrt_ssr_write(SNRT_SSR_DM1, SNRT_SSR_1D, out_copy + pooled_size - padded_kernels + i);
    ...
    snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_3D, in);
    snrt_ssr_write(SNRT_SSR_DM1, SNRT_SSR_2D, out);
  ...
  for (int step = 0; step < total_outs; ++step) {
    ...
    int h_index;
    #if USE_SSR_FREP_1D_SINGLE_CORE || USE_SSR_FREP_ALL_SINGLE_CORE
...
void MAXPOOL_FN_SINGLE_CORE_2D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs) {
  ...
  if (attr->pads[0] == 0 && attr->pads[1] == 0 && attr->pads[2] == 0 && attr->pads[3] == 0 &&
    ...
    snrt_ssr_read(SNRT_SSR_DM0, SNRT_SSR_4D, in);
    snrt_ssr_write(SNRT_SSR_DM1, SNRT_SSR_2D, out);
  ...
  for (int step = 0; step < total_outs; ++step) {
    ...
    #if USE_SSR_FREP_2D_SINGLE_CORE || USE_SSR_FREP_ALL_SINGLE_CORE
...
void MAXPOOL_FN_SINGLE_CORE_3D(maxpool_attributes* attr, double* in, double* out, int* idx, int total_outs) {
  ...
  for (int step = 0; step < total_outs; ++step) {
    ...
    #if USE_SSR_FREP_3D_SINGLE_CORE || USE_SSR_FREP_ALL_SINGLE_CORE
